...
OpenGLResource createTextureArrayFromDir(const std::vector<fs::path>& aTexturePaths) {
				if (aTexturePaths.empty()) {
								throw std::runtime_error("Texture array cannot be empty.");
				}

				GLsizei layersCount = static_cast<GLsizei>(aTexturePaths.size());

				bool isImageDataSet = false;
				int width = 0;
				int height = 0;
				...
				OpenGLResource textureArray = createTexture();
				GL_CHECK(glBindTexture(GL_TEXTURE_2D_ARRAY, textureArray.get()));
				GL_CHECK(glPixelStorei(GL_UNPACK_ALIGNMENT, 1));

				for (int i = 0; i < layersCount; i++) {
								auto textureFile = aTexturePaths[i];
								...
								if (!rawData) {
												throw OpenGLError("Failed to load image: " + textureFile.string());
								}

								std::unique_ptr<unsigned char, void(*)(void*)> data(rawData, stbi_image_free);

								if (isImageDataSet) {
												if (width != imageWidth || height != imageHeight) {
																throw std::runtime_error("Invalid texture size: all texture array layers must have the same size.");
												}
								}
								else {
												width = imageWidth;
												height = imageHeight;
												originalChannels = imageChannels;
												isImageDataSet = true;

												GL_CHECK(glTexStorage3D(GL_TEXTURE_2D_ARRAY, mipLevels, internalFormat, width, height, layersCount));
								}

								GL_CHECK(glTexSubImage3D(GL_TEXTURE_2D_ARRAY, 0, 0, 0, i, width, height, 1, format, GL_UNSIGNED_BYTE, data.get()));
				}
